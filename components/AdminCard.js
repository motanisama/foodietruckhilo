import {
  Heading,
  Avatar,
  Box,
  Center,
  Text,
  Stack,
  Button,
  Link,
  Badge,
  useColorModeValue,
  useToast,
} from "@chakra-ui/react";
import { useAuth } from "../lib/auth";
import { updateLocation } from "../lib/db";
import format from "date-fns/format";
import parseISO from "date-fns/parseISO";

export default function AdminCard({ user, updates, foodTruckData }) {
  const toast = useToast();

  const handleUpdate = async () => {
    const update = await updateLocation(user.truckId);
    toast({
      title: "Success!",
      description: "Updated Food Truck Location",
      status: "success",
      duration: 5000,
      isClosable: true,
    });
  };

  const color = useColorModeValue("gray.50", "gray.800");

  return (
    <Center h={"full"}>
      <Box
        w={"full"}
        bg={useColorModeValue("white", "gray.900")}
        boxShadow={"2xl"}
        rounded={"lg"}
        p={6}
        textAlign={"center"}
      >
        <Avatar
          size={"xl"}
          src={
            "https://images.unsplash.com/photo-1520810627419-35e362c5dc07?ixlib=rb-1.2.1&q=80&fm=jpg&crop=faces&fit=crop&h=200&w=200&ixid=eyJhcHBfaWQiOjE3Nzg0fQ"
          }
          alt={"Avatar Alt"}
          mb={4}
          pos={"relative"}
          _after={{
            content: '""',
            w: 4,
            h: 4,
            bg: "green.300",
            border: "2px solid white",
            rounded: "full",
            pos: "absolute",
            bottom: 0,
            right: 3,
          }}
        />
        <Heading fontSize={"2xl"} fontFamily={"body"}>
          @{foodTruckData && foodTruckData[0].truckName}
        </Heading>
        <Text size={"sm"} color="gray.400">
          Last updated:
          {updates && format(parseISO(updates[0].createdAt), "PPpp")}
        </Text>
        <Text size={"sm"} color="gray.400"></Text>
        <Text
          textAlign={"center"}
          color={useColorModeValue("gray.700", "gray.400")}
          px={3}
        ></Text>

        <Stack align={"center"} justify={"center"} direction={"row"} mt={6}>
          {foodTruckData &&
            foodTruckData[0].tag.map((tag, i) => (
              <Badge px={2} py={1} bg={color} fontWeight={"400"} key={i}>
                {`#${tag}`}
              </Badge>
            ))}
        </Stack>

        <Stack mt={8} direction={"row"} spacing={4}>
          <Button
            flex={1}
            fontSize={"sm"}
            rounded={"full"}
            _focus={{
              bg: "gray.200",
            }}
            onClick={handleUpdate}
          >
            Update Location
          </Button>
        </Stack>
      </Box>
    </Center>
  );
}
